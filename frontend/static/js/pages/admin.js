function deadlineFields(prefix) {
  const submission = $(`${prefix}SubmissionDeadline`).value;
  const voting = $(`${prefix}VotingDeadline`).value;
  if (!submission && !voting) return { submission_deadline: null, voting_deadline: null };
  if (!submission || !voting || voting <= submission) {
    throw new Error("Preenche ambos os prazos; a votação deve terminar depois das submissões.");
  }
  return { submission_deadline: submission, voting_deadline: voting };
}

function localDeadline(timestamp) {
...
function renderCurrent(week) {
  ...
  $("editVotingDeadline").value = localDeadline(week?.voting_deadline);
  $("saveDeadlines").disabled = !week;
  ...
  if (status) {
    ...
    status.innerHTML = `
      <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center">
        ...
        <span class="badge">${(week.films || []).reduce((total, film) => total + (film.votes || 0), 0)} votos registados</span>
        <span class="badge">${({submissions:"Submissões", voting:"Votação", paused:"Votação pausada", voting_closed:"Votação terminada", closed:"Fechada"})[week.phase] || "—"}</span>
        ${reviewFlag}
      </div>
    `;
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  $("saveDeadlines")?.addEventListener("click", async () => {
    if (!currentWeekData) return;
    const btn = $("saveDeadlines");
    try {
      const deadlines = deadlineFields("edit");
      setBusy(btn, true, "A guardar…");
      const week = await apiPost(`/admin/weeks/${currentWeekData.id}/deadlines`, deadlines, { auth: true });
      renderCurrent(week);
      toast("Prazos guardados.", "success");
  ...
  $("createWeek")?.addEventListener("click", async () => {
    ...
    try {
      ...
      await apiPost("/admin/weeks", { title, theme, ...deadlineFields("new") }, { auth: true });
      $("newSubmissionDeadline").value = "";
      $("newVotingDeadline").value = "";
      $("newWeekTitle").value = "";
