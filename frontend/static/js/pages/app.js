let _displayedWeek = null;
let _deadlineTimer = null;
function renderDeadlines(week) {
  _displayedWeek = week;
  clearTimeout(_deadlineTimer);
  const status = el("weekStatus");
  if (!status) return;
  let line = el("weekDeadlines");
  if (!line) {
    line = document.createElement("p");
    line.id = "weekDeadlines";
    line.className = "muted small";
    status.insertAdjacentElement("afterend", line);
  }
  const format = value => new Date(value * 1000).toLocaleString("pt-PT", {
    day:"2-digit", month:"short", hour:"2-digit", minute:"2-digit", timeZone:"Europe/Lisbon"
  });
  line.textContent = week.phase === "submissions" && week.submission_deadline != null
    ? `Submissões até ${format(week.submission_deadline)}`
    : ["voting", "paused"].includes(week.phase) && week.voting_deadline != null
      ? `Votação até ${format(week.voting_deadline)}` : "";
  const next = [week.submission_deadline, week.voting_deadline].find(value => value != null && value > week.server_time);
  if (next && week.is_open) {
    _deadlineTimer = setTimeout(() => load(), Math.min((next - week.server_time) * 1000 + 250, 2147483647));
  }
}
...
function prettyVoteError(e) {
  ...
  if (s === 403) return "Não podes votar neste filme.";
...
function prettySubmitError(e) {
  ...
  if (m.includes("PARTICIPATION_BANNED")) return "A tua participação está temporariamente restringida.";
  if (m.includes("Submissions are closed")) return "A fase de submissões terminou. Podes votar quando a votação estiver aberta.";
...
function setWeekStatus(week, alreadyVoted) {
  ...
  } else if (!week.is_open || week.phase === "voting_closed") {
    box.dataset.state = "closed";
    box.textContent = "Votação encerrada.";
  } else if (!week.is_ready) {
    box.dataset.state = "pending";
    box.textContent = week.phase === "paused" ? "Votação pausada." : "Submissões abertas — a votação começa depois.";
  ...
  return `
    <div class="${posterClass}" data-letterboxd="${escapeHtml(letterboxdUrl)}" data-title="${escapeHtml(f.title)}">
      ...
      <div class="poster-overlay">
        <button type="button" class="poster-btn poster-btn--know" data-film-details data-film-id="${escapeHtml(f.id)}" data-tmdb-id="${escapeHtml(f.tmdb_id || '')}" data-title="${escapeHtml(f.title)}" data-year="${escapeHtml(f.year || '')}" data-poster="${escapeHtml(f.poster_url || '')}" data-director="${escapeHtml(f.director || '')}">
...
function filmCard(week, f, alreadyVoted) {
  ...
  const isMyVote = String(localStorage.getItem(votedKey(week.id))) === String(f.id);
  div.className = "film-card fade-in" + (isMyVote ? " is-my-vote" : "");
...
function render(week) {
  ...
  if (openSubmission) {
    ...
    openSubmission.textContent = week.submissions_open ? "+ Submeter filme" : "Submissões encerradas";
  }
  const filmsHeading = el("films")?.closest("section")?.querySelector("h2");
  if (filmsHeading) filmsHeading.textContent = week.submissions_open ? "Os candidatos da semana" : week.voting_open ? "Em votação esta semana" : "A seleção da semana";
  ...
  if (el("subBtn")) el("subBtn").disabled = !week.submissions_open;
  const submitHint = el("submitHint");
  if (submitHint) {
    if (!getToken()) submitHint.textContent = "Faz login para submeter e votar.";
    else if (!week.submissions_open) submitHint.textContent = "Submissões encerradas. Não precisas de ter submetido para votar.";
...
async function submitFilmCurrentWeek() {
  ...
  if (!week?.submissions_open) { toast("Submissões encerradas — podes participar na votação.", "error", 4500); return; }
  ...
  try {
    ...
    btn.disabled = !_displayedWeek?.submissions_open;
...
function applyTheme(theme) {
  ...
  if (theme === "italian") {
    ...
    if (!document.querySelector(".netflix-brand-mark")) {
      ...
      brandMark.src = "/static/images/netflix-n.png";
      brandMark.alt = "";
