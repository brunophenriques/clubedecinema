...
async function loadLeaderboard() {
  try {
    ...
    el("lbPodium").innerHTML = top3.map((r, i) => `
      <div class="lb-podium__item lb-podium__item--${i + 1}">
        ${podiumAvatarHTML(r, i === 0 ? 80 : 64)}
        <a href="/profile/${escapeHtml(r.username)}" class="lb-podium__name">@${escapeHtml(r.username)}</a>
        <div class="lb-podium__wins">${r.films_won} vitória${r.films_won !== 1 ? "s" : ""} · ${r.win_rate}%</div>
        <div class="lb-podium__block">${RANK_EMOJI[i]}</div>
      </div>
    `).join("");

    // Table
    el("lbTableBody").innerHTML = rows.map((r, i) => `
      <tr>
        <td class="lb-table__rank lb-table__rank--${i < 3 ? i + 1 : ""}">${i < 3 ? RANK_EMOJI[i] : i + 1}</td>
        <td>
